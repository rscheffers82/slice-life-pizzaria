...
_cart.post = (data, callback) => {
    ...
    const email = helpers.validateEmail(payload.email) ? payload.email : false;
    const itemId = typeof(payload.itemId) === 'string' && payload.itemId.trim().length ===  14 ? payload.itemId : false
                ...
                if(!err && menuData) {
                    ...
                    _data.read('users', `${email}-cart`, (err, cartData) => {
                        if(!err && cartData) {
                            ...
                            const cartOverview = cartData.map(item => {
                                const {id, amount} = item;
                                const topUpPrice = ((item.size - 10) / 2) * menuData[id].sizeUpPrice;
