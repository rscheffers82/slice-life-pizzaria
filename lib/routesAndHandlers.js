...
// Get user details
// Route            GET api/users       query string
...
_users.put = function(data, callback) {
    ...
    if (!email) {
        callback(400, {error: 'Request should include email in its request body'});
        return;
    }
    ...
    _tokens.verifyToken(token, email, function(tokenIsValid) {
        if (tokenIsValid) {
            ...
            callback(403, {error: 'Token is invalid or no match between token and email'});
        }
    });
};

// Delete a user
// Route            DELETE api/users    query string
// Required data    email
// Optional data    none
...
_tokens.post = function(data, callback) {
    ...
    if (!password) {
        callback(400, {error: 'Request should include a password and it should meet the minimum requirements'});
        return;
    }
...
_menu.get = (data, callback) => {
    ...
    const email = helpers.validateEmail(queryStringObject.email) ? queryStringObject.email : false;
    const token = typeof(data.headers.token) === 'string' ? data.headers.token : false;
...
_cart.post = (data, callback) => {
    ...
    const id = typeof(payload.id) === 'string' && payload.id.trim().length ===  14 ? payload.id : false
    const size = [10, 12, 14, 16, 18].includes(payload.size) ? payload.size : false;    // 10: Small, 12: Medium, 14: Large, 16: Family, 18: Party
    ...
    if (!size) {
        callback(400, {error: 'Size missing or invalid (available options: 10, 12, 14, 16, 18)'});
        return;
    }
            ...
            _data.read('menu', 'menu', (err, menuData) => {
                if(!err && menuData) {
                    ...
                    _data.read('users', `${email}-cart`, (err, cartData) => {
                        if(!err && cartData) {
                            const cartOverview = helpers.createCartOverview(cartData, menuData);
                    ...
                    callback(400, {error: 'Generating a cart overview failed, could not load menu'});
                }
            });
        } else {
            callback(403, {error: 'Token is invalid or no match between token and email'});
...
_cart.delete = (data, callback) => {
    const {queryStringObject: query, headers} = data;
    const email = helpers.validateEmail(query.email) ? query.email : false;
    query.index = parseFloat(query.index);
    const index = !isNaN(query.index) && query.index % 1 === 0 ? query.index : false;
    const token = typeof(headers.token) === 'string' ? headers.token : false;
                ...
                if (!err && cartData) {
                    ...
                    _data.update('users', email + '-cart', updatedCart, err => {
                        if (!err) {
                            ...
                            callback(400, {error: 'Unable to clear cart. Please try again'});
                        }
                    });        
                                ...
                                if(!err && userData) {
                                    ...
                                    helpers.createStripePayment(order.id, order.total, err => {
                                        if(!err) {
                                            ...
                                            helpers.sendConfirmationMail(order.id, email, emailBody, err => {
...
handlers.preflight = (data, callback) => {
    // In preflight return statusCode 204
    // allowed origin will be set in chosenHandler
    callback(204, undefined, 'plain');
};
